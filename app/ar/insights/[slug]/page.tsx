...
export function generateStaticParams() {
  return insights.filter((a) => a.locale === "ar").map((a) => ({ slug: a.slug }));
}

export async function generateMetadata({ params }: InsightParams): Promise<Metadata> {
  const { slug } = await params;
  const article = getInsight(slug, "ar");
  if (!article) return {};
  return {
    ...
    alternates: {
      ...
      ...(article.counterpartPath
        ? {
            languages: {
              en: article.counterpartPath,
              ar: article.path,
              "x-default": article.counterpartPath
            }
          }
        : {})
    },
...
export default async function InsightPageAr({ params }: InsightParams) {
  const { slug } = await params;
  const article = getInsight(slug, "ar");
  if (!article) notFound();
