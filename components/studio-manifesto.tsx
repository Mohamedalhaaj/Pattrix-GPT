  ...
  return (
    <section ref={ref} id="studio" className="relative overflow-hidden bg-ink py-24 text-white sm:py-32">
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8">
        <Reveal>
          <p className="mb-8 text-xs font-semibold uppercase text-white/50">Studio position</p>
          <h2 className="max-w-6xl font-display text-[clamp(3.5rem,10vw,10rem)] font-semibold leading-[0.86] text-balance">
            Quiet luxury for brands that want to be remembered.
          </h2>
        </Reveal>
        <div className="mt-16 grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
          <Reveal className="dark-glass rounded-[2rem] p-6 sm:p-8">
            <p className="font-editorial text-2xl leading-snug text-white/82 sm:text-3xl">
              We design less noise and more gravity: the kind of digital presence that slows the room down.
            </p>
          </Reveal>
          <Reveal delay={0.12}>
            <p className="max-w-2xl text-pretty text-lg leading-8 text-white/62 lg:ml-auto">
              Pattrix combines strategy, production, and interface craft into one calm system. Every touchpoint is composed for hierarchy, story, and finish before it becomes content.
            </p>
          </Reveal>
        </div>
      </div>
      ...
      >
        <span>PATTRIX / CINEMATIC MINIMALISM / </span>
        <span>PATTRIX / CINEMATIC MINIMALISM / </span>
      </motion.div>
    </section>
  );
